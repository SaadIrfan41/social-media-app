  ...
  return (
    <div>
      <div className='w-full px-4 sm:px-6 '>
        <div className='flex justify-between border-b-2 border-gray-100 py-4 items-center  mx-auto'>
          <div className='flex'>
            <Link href='/'>
              <a className='flex items-center h-10 w-40 flex-shrink-0 text-indigo-500'>
                <svg
                  className=''
          ...
          <div>
            <div className='mt-1 ui loading icon input relative w-96 '>
              <input
                className={'w-full p-5 h-10 sm:text-sm  rounded-md'}
                onChange={(e) => setname(e.target.value)}
                placeholder='Search Users...'
              />
              {loading && <i aria-hidden='true' className='spinner icon'></i>}
              {data?.filterUsers && (
                <div className='rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 overflow-hidden mt-12 absolute w-96'>
                  <div className='relative grid gap-3 bg-white px-5 py-6  sm:gap-3 sm:p-8 divide-y divide-gray-200'>
                    {data?.filterUsers?.map((user: any) => (
                      <a
                        key={user.id}
                        href='#'
                        className='flex-shrink-0 group block hover:bg-gray-100 rounded-md w-full p-2 '
                      >
                        <div className='flex items-center '>
                          ...
                          <div className='ml-10'>
                            <p className='text-lg font-medium  text-gray-700 group-hover:text-gray-900 capitalize '>
                              {user.name}
                            </p>
                          </div>
                        </div>
          ...
          <div className='flex'>
            {status === 'unauthenticated' && (
              <>
                <div className=' px-2'>
                  <Link href='/login'>
                    <a className=' space-x-10 flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-lg font-xl text-indigo-500 hover:text-white  bg-white border-indigo-400 hover:bg-indigo-600 ransition duration-500 ease-in-out'>
                      Sign In
                    </a>
                  </Link>
                </div>
                <div>
                  <Link href='/register'>
                    <a className=' space-x-10 flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-lg font-medium text-indigo-500 hover:text-white  bg-white border-indigo-400 hover:bg-indigo-600 ransition duration-500 ease-in-out'>
                      Sign Up
                    </a>
                  </Link>
                </div>
